// Mock callback for clearstone_core.flash_swap_pt and flash_swap_sy — TEST ONLY.
//
// Mode byte (first byte of `data`) selects behavior. Same encoding for both
// directions; on the buy side the repay leg moves SY from solver to escrow,
// on the sell side it moves PT.
//   0 = NoOp        — don't repay (core must revert with FlashRepayInsufficient)
//   1 = Ok          — transfer the required amount from solver's src ATA to market escrow
//   2 = ShortRepay  — transfer required - 1 (triggers FlashRepayInsufficient)
//   3 = TryNestedFlash — CPI core.flash_swap_pt recursively (NestedFlashBlocked)
//
...
pub mod mock_flash_callback {
    ...
    pub fn on_flash_sy_received(
        ...
        match mode {
            MODE_NOOP => Ok(()),
            MODE_OK => repay_pt(&ctx, pt_required),
            MODE_SHORT_REPAY => {
                let short = pt_required
                    .checked_sub(1)
                    .ok_or(MockError::MissingModeByte)?;
                repay_pt(&ctx, short)
            }
            MODE_TRY_NESTED_FLASH => {
                // Same shape as the buy-side nested test: any nested flash
                // (buy or sell) on this market must be rejected by the
                // shared `flash_pt_debt` guard. CPI flash_swap_pt with a
                // dummy 1-PT borrow; core will revert before any state
                // moves.
                let ix = clearstone_core::cpi::accounts::FlashSwapPt {
                    caller: ctx.accounts.solver.to_account_info(),
                    market: ctx.accounts.market.to_account_info(),
                    caller_pt_dst: ctx.accounts.solver_pt_src.to_account_info(),
                    token_sy_escrow: ctx.accounts.token_sy_escrow.to_account_info(),
                    token_pt_escrow: ctx.accounts.token_pt_escrow.to_account_info(),
                    token_fee_treasury_sy: ctx.accounts.token_fee_treasury_sy.to_account_info(),
                    mint_sy: ctx.accounts.mint_sy.to_account_info(),
...
pub struct OnFlashPtReceived<'info> {
    ...
    /// CHECK: passed to the nested flash_swap_pt CPI (PT mint, required after
...
fn repay_pt<'info>(ctx: &Context<OnFlashSyReceived<'info>>, amount: u64) -> Result<()> {
    if amount == 0 {
        return Ok(());
    }
    transfer_checked(
        CpiContext::new(
            ctx.accounts.core_token_program.to_account_info(),
            TransferChecked {
                from: ctx.accounts.solver_pt_src.to_account_info(),
                mint: ctx.accounts.mint_pt.to_account_info(),
...
pub struct OnFlashSyReceived<'info> {
    // -- Fixed 6-account prefix core injects (mirrors OnFlashPtReceived
    //    with caller_sy_dst + token_pt_escrow swapped in for the prefixes
    //    that hold the borrowed leg / the repayable leg) --
    /// CHECK: market account (readonly here).
    pub market: UncheckedAccount<'info>,
    ...
    #[account(mut)]
    pub solver_pt_src: Box<InterfaceAccount<'info, TokenAccount>>,
